...
export class SignatureCanvas extends Component<SignatureCanvasProps> {
    ...
    static refNullError = new Error("react-signature-canvas is currently " + "mounting or unmounting: React refs are null during this phase.")
    ...
    override render: Component["render"] = () => {
        const { canvasProps, className } = this.props
        const mergedClassName = [className, canvasProps?.className].filter(Boolean).join(" ")

        return <canvas ref={this.setRef} {...canvasProps} className={mergedClassName} />
    }

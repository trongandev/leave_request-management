        ...
        <div className="flex-1 flex flex-col gap-8">
            <section className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                <div className="mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                    <h3 className="text-slate-900 dark:text-white text-lg font-bold">{t("preferences.security.password.title")}</h3>
                    <p className="text-slate-500 text-sm mt-1">{t("preferences.security.password.desc")}</p>
                </div>
                <form className="max-w-xl space-y-5">
                    ...
                    <div className="text-xs text-slate-500 pt-2">
                        <p className="font-medium text-slate-700 dark:text-slate-400 mb-1">{t("preferences.security.password.requirements.title")}</p>
                        <ul className="list-disc pl-4 space-y-0.5">
                            <li>{t("preferences.security.password.requirements.minLength")}</li>
                            <li>{t("preferences.security.password.requirements.lowercase")}</li>
                            <li>{t("preferences.security.password.requirements.symbol")}</li>
                        </ul>
                    </div>
                </form>
            </section>
                ...
                <div className="flex items-start justify-between">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <h3 className="text-slate-900 dark:text-white text-lg font-bold">{t("preferences.security.twoFactor.title")}</h3>
                            <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 uppercase tracking-wide">{t("preferences.security.twoFactor.enabled")}</span>
                        </div>
                        <p className="text-slate-500 text-sm max-w-2xl">{t("preferences.security.twoFactor.desc")}</p>
                    </div>
            ...
            <section className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6 shadow-sm mb-10">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h3 className="text-slate-900 dark:text-white text-lg font-bold">{t("preferences.security.loginActivity.title")}</h3>
                        <p className="text-slate-500 text-sm">{t("preferences.security.loginActivity.desc")}</p>
                    </div>
                    <button className="text-sm font-medium text-red-600 hover:text-red-700 transition-colors">{t("preferences.security.loginActivity.signOutAll")}</button>
                </div>
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                <p className="text-sm font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                                    MacBook Pro 16"
                                    <span className="px-1.5 py-0.5 rounded text-[10px] bg-green-100 text-green-700 font-bold border border-green-200">{t("preferences.security.loginActivity.current")}</span>
                                </p>
                                <p className="text-xs text-slate-500 mt-0.5">{t("preferences.security.loginActivity.devices.macbook")}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-xs font-medium text-emerald-600 flex items-center gap-1 justify-end">
                                <span className="size-2 rounded-full bg-emerald-500"></span> {t("preferences.security.loginActivity.activeNow")}
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800 last:border-0">
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-0.5">{t("preferences.security.loginActivity.devices.iphone")}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-xs text-slate-500">{t("preferences.security.loginActivity.times.twoHoursAgo")}</p>
                        </div>
                    </div>
                    <div className="flex items-center justify-between py-3 last:border-0">
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-0.5">{t("preferences.security.loginActivity.devices.windows")}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-xs text-slate-500">{t("preferences.security.loginActivity.times.threeDaysAgo")}</p>
                        </div>
                    </div>
                </div>
            </section>
            <div className="flex items-center justify-end gap-3 pb-20">
                <button className="px-6 py-2 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">{t("preferences.security.actions.discard")}</button>
                <button className="px-8 py-2 text-sm font-semibold text-white bg-primary hover:bg-primary/90 rounded-lg shadow-lg shadow-primary/20 transition-all">{t("preferences.security.actions.update")}</button>
            </div>
        </div>
    )
}

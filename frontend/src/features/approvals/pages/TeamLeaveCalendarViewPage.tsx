...
export default function TeamLeaveCalendarViewPage() {
	...
	const teamMembers = Array.isArray(data?.teamMember)
		? data.teamMember.filter(Boolean)
		: [];
	const requestAllTeamMembers = useMemo(() => {
		return Array.isArray(data?.getRequestAllTeamMembers)
			? data.getRequestAllTeamMembers.filter(Boolean)
			: [];
	}, [data]);
	...
	const requestsByDate = useMemo(() => {
		...
		requestAllTeamMembers.forEach((request) => {
			...
			while (cursor.isBefore(endDate) || cursor.isSame(endDate, "day")) {
				...
				const hasCreator = grouped[dateKey].some(
					(item) => item.creatorId === creatorInfo.creatorId,
				);
				if (!hasCreator) {
	...
	const handleExportExcel = () => {
		const rows = requestAllTeamMembers
			.map((request) => {
				...
				const normalizedStart = startDate.isAfter(endDate)
					? endDate
					: startDate;
		...
		XLSX.writeFile(
			workbook,
			`team_leave_calendar_${viewDate.format("YYYY_MM")}.xlsx`,
		);
	};
	return (
				...
				<aside className="w-72 bg-card flex flex-col z-10  md:flex shadow-xs rounded-lg border">
					<div className="p-5 border-b">
						<h2 className="text-sm font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-4">
							{t("approvals.calendar.filters.title")}
						</h2>
					...
					<div className="px-5 py-4 border-b bg-neutral-50/50 dark:bg-neutral-700/50">
						<h3 className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 mb-3 uppercase">
							{t("approvals.calendar.filters.legendTitle")}
						</h3>
					...
					<div className="flex-1 custom-scrollbar p-5">
						<div className="flex items-center justify-between mb-3">
							<h3 className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase">
								{t("approvals.calendar.teamMembers.title")}
							</h3>
							<button className="text-xs text-primary hover:text-primary-dark font-medium">
								{t("approvals.calendar.teamMembers.selectAll")}
							</button>
						</div>
						<div className="space-y-3  overflow-y-auto h-[400px] pr-1">
							{teamMembers.map((member, index) => (
								...
								>
									<input
										className="rounded border-neutral-300 text-primary focus:ring-primary/30 h-4 w-4"
										type="checkbox"
									/>
									...
									<div className="flex-1">
										<p className="text-sm font-medium text-neutral-700 dark:text-neutral-200 group-hover:text-primary transition-colors">
											{member?.fullName || "Unknown member"}
										</p>
										<p className="text-xs text-neutral-400">
											{member?.departmentId?.originName || "No department"}
										</p>
									</div>
								</label>
							))}
					...
					<div className="flex-1 bg-card  rounded-xl border shadow-xs overflow-hidden flex flex-col">
						<div className="grid grid-cols-7  ">
							{["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"].map((d) => (
								<div
									key={d}
									className={` text-center text-sm w-full h-12 flex items-center justify-center  bg-foreground/5 font-semibold uppercase tracking-wide text-neutral-500`}
								>
									{d}
								</div>
							))}

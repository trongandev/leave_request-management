            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    <div className="flex items-center gap-4 p-4 bg-primary-50 dark:bg-primary/10 rounded-lg border border-primary-100 dark:border-primary/20">
                        <CAvatarProfile
                            user={{ ...(adjustEmp?.userId as any), avatar: userMap[adjustEmp?.userId?._id || ""]?.avatar || adjustEmp?.userId?.avatar }}
                            className="w-12 h-12 border-2 border-white dark:border-neutral-700"
                        />
                        <div>
                    ...
                    <form className="space-y-5">
                        <div>
                            ...
                            <div className="flex p-1.5 bg-muted/50 rounded-xl border border-border">
                                <label className="flex-1 cursor-pointer">
                                    <input className="peer sr-only" name="adj-type" type="radio" checked={adjustType === "add"} onChange={() => setAdjustType("add")} />
                                ...
                                <label className="flex-1 cursor-pointer">
                                    <input className="peer sr-only" name="adj-type" type="radio" checked={adjustType === "deduct"} onChange={() => setAdjustType("deduct")} />
                        ...
                        <div className="pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-lg font-bold text-primary dark:text-blue-400 font-mono">
                                    {isNaN(previewDays) ? adjustEmp?.remainingDays : previewDays.toFixed(1)} {t("admin.employees.adjustModal.days")}
                                </span>
                            </div>
                        </div>
                    </form>
                                ...
                                logsData.data.map((log: any) => (
                                    <div key={log.id || log._id} className="ml-4 relative">
                                        ...
                                        <p className={`text-xs font-mono ${(log.changeAmount || 0) > 0 ? "text-green-500" : "text-red-500"}`}>
                                            {(log.changeAmount || 0) > 0 ? "+" : ""}
                                            {log.changeAmount} {t("admin.employees.adjustModal.days")}
                                        </p>
                                    </div>
                                ))
                            ) : (
                ...
                <div className="p-6 border-t border-border bg-card mt-auto">
                    <div className="flex gap-3">
                        ...
                        <Button className="flex-1 gap-2 h-11 font-bold shadow-lg shadow-primary/20" onClick={handleSaveChanges} disabled={isAdjusting}>
                            {isAdjusting ? <span className="size-4 rounded-full border-2 border-primary-foreground border-t-transparent animate-spin"></span> : <SaveIcon className="size-4" />}{" "}
                            {t("admin.employees.adjustModal.save")}
                        </Button>
                    </div>
                </div>
            </div>
                            ...
                            <div className="w-full md:w-[35%] p-6 border-r border-border bg-muted/10 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
                                <div>
                                    ...
                                    <div className="flex p-1 bg-muted/50 rounded-xl border border-border">
                                        <label className="flex-1 cursor-pointer">
                                            <input className="peer sr-only" name="export-format" type="radio" checked={exportFormat === "csv"} onChange={() => setExportFormat("csv")} />
                                            <div className="text-center py-3 px-2 rounded-lg transition-all flex flex-col items-center gap-2 group peer-checked:bg-card peer-checked:shadow-sm">
                                                <svg
                                                    ...
                                                    stroke-width="2"
                                                    className={`w-8 h-8 transition-opacity ${exportFormat === "csv" ? "opacity-100" : "opacity-40 grayscale group-hover:opacity-70"}`}
                                                >
                                                    <path
                                                        stroke="#979593"
                                                ...
                                                <span
                                                    className={`text-[10px] font-bold uppercase tracking-wider transition-colors ${exportFormat === "csv" ? "text-primary" : "text-muted-foreground"}`}
                                                >
                                                    CSV
                                                </span>
                                            </div>
                                        ...
                                        <label className="flex-1 cursor-pointer">
                                            <input className="peer sr-only" name="export-format" type="radio" checked={exportFormat === "xlsx"} onChange={() => setExportFormat("xlsx")} />
                                            <div className="text-center py-3 px-2 rounded-lg transition-all flex flex-col items-center gap-2 group peer-checked:bg-card peer-checked:shadow-sm">
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    className={`w-8 h-8 transition-opacity ${exportFormat === "xlsx" ? "opacity-100" : "opacity-40 grayscale group-hover:opacity-70"}`}
                                                    viewBox="0 0 32 32"
                                                >
                                                    <defs>
                                                        <linearGradient
                                                ...
                                                <span
                                                    className={`text-[10px] font-bold uppercase tracking-wider transition-colors ${exportFormat === "xlsx" ? "text-primary" : "text-muted-foreground"}`}
                                                >
                                                    XLSX
                                                </span>
                                            </div>
                                        ...
                                        <label className="flex-1 cursor-pointer">
                                            <input className="peer sr-only" name="export-format" type="radio" checked={exportFormat === "pdf"} onChange={() => setExportFormat("pdf")} />
                                            <div className="text-center py-3 px-2 rounded-lg transition-all flex flex-col items-center gap-2 group peer-checked:bg-card peer-checked:shadow-sm">
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    className={`w-8 h-8 transition-opacity ${exportFormat === "pdf" ? "opacity-100" : "opacity-40 grayscale group-hover:opacity-70"}`}
                                                    viewBox="0 0 24 24"
                                                >
                                                    <path
                                                        fill="#ef5350"
                                                ...
                                                <span
                                                    className={`text-[10px] font-bold uppercase tracking-wider transition-colors ${exportFormat === "pdf" ? "text-primary" : "text-muted-foreground"}`}
                                                >
                                                    PDF
                                                </span>
                                            </div>
                                        ...
                                        <thead className="bg-muted/50 text-muted-foreground uppercase pointer-events-none sticky top-0 z-10">
                                            <tr>
                                                {exportDataOptions.length > 0 &&
                                                    Object.keys(exportDataOptions[0]).map((key) => (
                                                        <th key={key} className="px-4 py-3 border-b border-r border-border font-bold tracking-tight">
                                                            {key}
                                                        </th>
                                                    ))}
                                            </tr>
                                            ...
                                            {exportDataOptions.map((row, idx) => (
                                                <tr key={idx} className="bg-card border-b border-border pointer-events-none hover:bg-muted/50 transition-colors">
                                                    {Object.values(row).map((val: any, vIdx) => (
                                                        <td key={vIdx} className="px-4 py-3 border-r border-border font-medium text-foreground">
                                                            {String(val)}
                                                        </td>
                                                    ))}
                                                </tr>
                                            ...
                                            {exportDataOptions.length === 0 && (
                                                <tr>
                                                    <td colSpan={7} className="px-4 py-8 text-center text-neutral-500">
                                                        {t("admin.employees.exportModal.noData", "No data")}
                                                    </td>
                                                </tr>
                                            )}
                                        </tbody>
                        ...
                        <div style={{ position: "absolute", left: "-9999px", top: "-9999px" }}>
                            <div id="export-pdf-container" style={{ width: "1000px", backgroundColor: "#ffffff", padding: "40px", fontFamily: "sans-serif" }}>
                                <div style={{ textAlign: "center", marginBottom: "30px", borderBottom: "2px solid #e5e7eb", paddingBottom: "20px" }}>
                                    <h1 style={{ fontSize: "28px", fontWeight: "bold", margin: "0 0 10px 0", color: "#111827", textTransform: "uppercase", letterSpacing: "1px" }}>
                                        {t("admin.employees.exportModal.pdfTitle", "Leave Balance Report")}
                                    </h1>
                                ...
                                <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "sans-serif" }}>
                                    <thead style={{ backgroundColor: "#f3f4f6", color: "#374151", textTransform: "uppercase", fontSize: "12px", textAlign: "left" }}>
                                        <tr>
                                            {exportDataOptions.length > 0 &&
                                                Object.keys(exportDataOptions[0]).map((key) => (
                                                    <th key={key} style={{ padding: "12px 16px", borderBottom: "1px solid #e5e7eb", borderRight: "1px solid #e5e7eb", fontWeight: "bold" }}>
                                                        {key}
                                                    </th>
                                                ))}
                                        </tr>
                                    ...
                                    <tbody style={{ fontSize: "14px", color: "#1f2937" }}>
                                        {exportDataOptions.map((row, idx) => (
                                            <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? "#ffffff" : "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
                                                {Object.values(row).map((val: any, vIdx) => (
                                                    <td key={vIdx} style={{ padding: "8px 16px", borderRight: "1px solid #e5e7eb" }}>
                                                        {String(val)}
                                                    </td>
                                                ))}
                                            </tr>

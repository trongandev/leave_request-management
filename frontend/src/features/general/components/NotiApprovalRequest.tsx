...
export default function NotiApprovalRequest({
	open,
	onOpenChange,
	data,
}: {
					...
					<div className="grid grid-cols-2 gap-4">
						...
						<div className="bg-white border rounded-lg p-3 flex items-start flex-col gap-1">
							...
							{data?.values?.startDate && (
								<p className="text-sm font-medium text-on-surface">
									{format(data?.values?.startDate, "dd/MM")} -{" "}
									{format(data?.values?.endDate, "dd/MM")} (
									{data?.values?.totalDays} ngày)
								</p>
							)}
						</div>
					</div>
